...
func (router *Route) Match(w http.ResponseWriter, r *http.Request) (rd *RouteData, matched bool) {
	if !router.inited {
		router.Init()
	}
	...
	md, ok := NamedRegexpGroup(url, router.rePath)
	if !ok {
		return
	}
...
func (rt *RouteTable) Match(w http.ResponseWriter, r *http.Request) (rd *RouteData, matched bool) {
	for _, router := range rt.Routes {
		if _rd, ok := router.Match(w, r); ok {
			rd = _rd
			matched = true
			break
		}
